...
/* ── SUPABASE CLIENT ─────────────────────────────────────────────────────
   ...
   ─────────────────────────────────────────────────────────────────────── */
/* Supabase credentials — env vars override if set, otherwise use defaults */
var SUPABASE_URL = (typeof import.meta !== "undefined" && import.meta.env && import.meta.env.VITE_SUPABASE_URL)
  ? import.meta.env.VITE_SUPABASE_URL
  : "https://cdnrujlpccamsyxzvydy.supabase.co";
var SUPABASE_ANON_KEY = (typeof import.meta !== "undefined" && import.meta.env && import.meta.env.VITE_SUPABASE_ANON_KEY)
  ? import.meta.env.VITE_SUPABASE_ANON_KEY
  : "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNkbnJ1amxwY2NhbXN5eHp2eWR5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE1NDg0NDgsImV4cCI6MjA5NzEyNDQ0OH0.qt_3YpO_BeVCXHSPAy10BIyLZXSfwm3oAI-bpqICOP0";
...
export default function App(){
  ...
  var [loading,setLoading]=useState(false);
  var [issLoad,setIssLoad]=useState(false);
  var [report,setReport]=useState(null);
  var [risks,setRisks]=useState([]);
  var [issues,setIssues]=useState([]);
  var [issErr,setIssErr]=useState("");
  var [err,setErr]=useState("");
  var [nav,setNav]=useState("projects");
  ...
  useEffect(function(){
    (async function(){
      ...
      try {
        ...
        /* Handle ALL Supabase callback patterns:
           1. ?access_token= (token directly in query params)
           2. ?code= (PKCE flow - exchange for token)
           3. #access_token= (implicit flow - token in hash) */
        var _url = new URL(window.location.href);
        var _searchParams = _url.searchParams;

        /* Pattern 1: access_token directly in query params */
        var _directToken = _searchParams.get("access_token");
        var _directRefresh = _searchParams.get("refresh_token");
        if(_directToken) {
          window.history.replaceState(null, "", window.location.pathname);
          localStorage.setItem("d365_token", _directToken);
          if(_directRefresh) localStorage.setItem("d365_refresh", _directRefresh);
          sbSetAuth(_directToken);
        }

        /* Pattern 2: PKCE code exchange */
        var authCode = _searchParams.get("code");
        if(authCode && !_directToken) {
          window.history.replaceState(null, "", window.location.pathname);
          try {
